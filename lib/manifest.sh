...
# manifest_skill_names — every skill name the skills manifest declares, sorted.
...
manifest_validate() {
  ...
  [[ -n "$bad" ]] && { problem "mcp.tsv names agents with no installer: $(tr '\n' ' ' <<< "$bad")"; problems=1; }
  ...
  [[ -n "$bad" ]] && { problem "skills.tsv declares a skill twice: $(tr '\n' ' ' <<< "$bad")"; problems=1; }
  bad=$(manifest_rows "$mcp" | cut -f1 | sort | uniq -d)
  [[ -n "$bad" ]] && { problem "mcp.tsv declares a server twice: $(tr '\n' ' ' <<< "$bad")"; problems=1; }
  bad=$(manifest_rows "$plugins" | awk -F'\t' '{print $4"@"$2}' | sort | uniq -d)
  [[ -n "$bad" ]] && { problem "plugins.tsv declares a plugin twice: $(tr '\n' ' ' <<< "$bad")"; problems=1; }
